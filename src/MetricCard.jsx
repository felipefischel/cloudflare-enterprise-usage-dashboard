...
import { Globe, Activity, Database, TrendingUp, TrendingDown, AlertCircle, Shield, CheckCircle, Network, Users } from 'lucide-react';
...
function MetricCard({ 
  ...
  const getIcon = () => {
    switch (icon) {
      ...
      case 'users':
        return <Users className={compact ? "w-5 h-5" : "w-6 h-6"} />;
      default:
        return <Activity className={compact ? "w-5 h-5" : "w-6 h-6"} />;
    }
  };
    ...
    >
      <div className={compact ? "p-4" : "p-6"}>
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {/* Top-right badge area: HTTP totals/blocked or zone breakdown */}
          {summaryBadge ? (
            <div className="ml-4">
              {summaryBadge}
            </div>
